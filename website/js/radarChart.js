...
function RadarChart(id, data, options) {
	...
	var allAxis = (data[0].map(function (i, j) { return i.axis })),	//Names of each axis
		...
		Format = function (d) { return d },			 	//Percentage formatting
		angleSlice = Math.PI * 2 / total;		//The width in radians of each "slice"

	var Format2 = function (d) {
		const gne = {
			"Acidity": ["Low Acidity", "Medium-low Acidity", "Medium Acidity", "Medium-high Acidity", "Basically a lemon"],
			"Tannins": ["No Tannins", "Low Tannins", "Medium Tannins", "Medium-high Tannins", "High Tannins"],
			...
			"Alcohol": ["Under 10% ABV", "10–11.5% ABV", "11.5–13.5% ABV", "13.5–15% ABV", "Over 15% ABV"]
	...
	var drag = d3.drag().on("drag", function (d, i) {
		...
		//Move the rest
		blobWrapper.selectAll(".radarArea, .radarStroke")
			.attr("d", d => radarLine(d))
		
	}).on("end", function (d, i) {
		d.value = Math.min(Math.round(d.value), maxValue)
		d3.select(this)
			.transition()
			.attr("cx", rScale(d.value) * Math.cos(angleSlice * i - Math.PI / 2))
			.attr("cy", rScale(d.value) * Math.sin(angleSlice * i - Math.PI / 2))
	...
	blobCircleWrapper.selectAll(".radarInvisibleCircle").call(
		drag
	)
